...
// Fetch the data only if it has not been fetched before
    if (!isDataFetched) {
      ...
      cachedData = data;
// the flag to true to avoid repeated fetching
      isDataFetched = true;
    }

    const planetInfo = cachedData.bodies.find(
      (body) => `planet${body.id}` === planetId
    );
...
function updatePlanetDescription(planetInfo) {
  ...
  const { name, latinName, desc, circumference, distance, temp, moons } =
    planetInfo;
  ...
  descElement.classList.add("planet-info", "planet-info--scrollable");
  ...
  circumferenceElement.innerHTML = `<span class="planet-info__titles">OMKRETS</span><br>${formatNumber(
    circumference
  )} km`;
  circumferenceElement.classList.add("planet-info");

  const distanceElement = document.createElement("p");
  distanceElement.innerHTML = `<span class="planet-info__titles">KM FRÅN SOLEN</span><br>${formatNumber(
    distance
  )} km`;
